...
export default function WritingSection({ onNext }: WritingSectionProps) {
  ...
  const handleSubmit = () => {
    if (answer.trim().length < 100)
      return alert("⚠️ Bài viết cần ít nhất 100 từ!");
    setIsSubmitted(true);
    alert("✅ Bài Writing của bạn đã được nộp thành công!");
    if (onNext) onNext(); // ✅ Gọi sang Speaking
  };
      ...
      <div className="max-w-4xl mx-auto bg-white shadow-xl rounded-3xl p-8 border border-[#B8D7F9]/50">
        ...
        <div className="bg-[#F8FBFF] p-6 rounded-2xl shadow-inner mb-8">
          <h2 className="text-lg font-medium text-gray-700 leading-relaxed">
            ...
            many large cities today. How should we deal with this problem? Write
            a single paragraph (100–150 words). Try to use words and grammar
            that will show the best of your English.
          </h2>
        </div>
